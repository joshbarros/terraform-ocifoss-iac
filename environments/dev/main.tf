 ...
 * with smaller, more cost-effective resources
 */
...
module "network" {
  ...
  open_ports                 = [22, 80, 443, 6443, 8080, 9000, 9001, 3000, 3100, 5678, 8090]
...
module "budget" {
  ...
  budget_description   = "Development budget to monitor credit usage (US$${var.budget_amount})"
...
resource "oci_ons_notification_topic" "alarms_topic" {
  compartment_id = oci_identity_compartment.compartment.id
  name           = "foss-stack-dev-alarms"
  description    = "Topic for FOSS stack development monitoring alarms"
}
...
module "monitoring" {
  ...
  cpu_alarm_display_name      = "FOSS-Stack-Dev-CPU-Alarm"
  memory_alarm_display_name   = "FOSS-Stack-Dev-Memory-Alarm"
  disk_alarm_display_name     = "FOSS-Stack-Dev-Disk-Alarm"
  ...
  alarm_body                  = "The FOSS Stack development instance has exceeded the resource usage threshold"
